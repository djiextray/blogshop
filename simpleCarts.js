...
function cart(a, b) {
    ...
    this.totalItems = 0, this.totalPrice = 0, this.totalWeight = 0, this.totalShip = 0, this.totalOrder = 0, this.items = new Array, this.userEmail = a, this.ItemColumns = ["Image", "Name", "Price", "Quantity", "Weight", "Total", "Remove"], this.initialize = function() {
        ...
        document.getElementsByClassName("simpleCart_booking").html(this.returnFormattedPrice(booking));
        ...
        for (d = getElementsByClassName("simpleCart_items"), a = 0; a < d.length; a++) {
            ...
            for (newRow.className = "cartHeaders", cartTable.appendChild(newRow), a = 0, a = 0; a < this.items.length; a++) {
                for (tempItem = this.items[a], newRow = document.createElement("div"), f = 0, f = 0; f < this.ItemColumns.length; f++) tempCell = document.createElement("div"), tempCell.className = "item" + this.ItemColumns[f], "Image" == this.ItemColumns[f] && tempItem.getValue("image") && (tempCell.innerHTML = '<img src="' + tempItem.getValue("image") + '" />'), "Name" == this.ItemColumns[f] ? tempCell.innerHTML = "<b>" + tempItem.getValue("name") + "</b>" + c(tempItem.getValue("type")) + c(tempItem.getValue("color")) : "Price" == this.ItemColumns[f] ? tempCell.innerHTML = this.returnFormattedPrice(tempItem.getValue("price")) : "Options" == this.ItemColumns[f] && this.options() ? tempCell.innerHTML = tempItem.optionList() : "Quantity" == this.ItemColumns[f] ? tempCell.innerHTML = '<input type="number" onblur="simpleCart.updateQuantity(' + tempItem.functionString() + ",'new_quantity=' + this.value);cekHarga();return false;\"value=\"" + tempItem.getValue("quantity") + '" />' : "Weight" == this.ItemColumns[f] ? tempCell.innerHTML = this.returnFormattedWeight(tempItem.getValue("weight")) + " kg" : "Total" == this.ItemColumns[f] ? tempCell.innerHTML = this.returnFormattedPrice(tempItem.getValue("quantity") * tempItem.getValue("price")) : "Remove" == this.ItemColumns[f] && (tempCell.innerHTML = '<a class="removeButton" onclick="simpleCart.updateQuantity(' + tempItem.functionString() + ",'new_quantity=0');cekHarga();return false;\">Remove</a>"), newRow.appendChild(tempCell);
        ...
        for (d = getElementsByClassName("simpleCart_table"), a = 0; a < d.length; a++) {
            ...
            for (newRow.className = "thead", cartTable.appendChild(newRow), a = 0, a = 0; a < this.items.length; a++) {
                for (tempItem = this.items[a], newRow = document.createElement("tr"), f = 0, f = 0; f < 6; f++) tempCell = document.createElement("td"), tempCell.className = "item" + this.ItemColumns[f], "Image" == this.ItemColumns[f] && tempItem.getValue("image") && (tempCell.innerHTML = '<img src="' + tempItem.getValue("image") + '" />'), "Name" == this.ItemColumns[f] ? tempCell.innerHTML = "<b>" + tempItem.getValue("name") + "</b>" + c(tempItem.getValue("type")) + c(tempItem.getValue("color")) : "Price" == this.ItemColumns[f] ? tempCell.innerHTML = this.returnFormattedPrice(tempItem.getValue("price")) : "Quantity" == this.ItemColumns[f] ? tempCell.innerHTML = tempItem.getValue("quantity") : "Weight" == this.ItemColumns[f] ? tempCell.innerHTML = this.returnFormattedWeight(tempItem.getValue("weight")) + " kg" : "Total" == this.ItemColumns[f] && (tempCell.innerHTML = this.returnFormattedPrice(tempItem.getValue("quantity") * tempItem.getValue("price"))), newRow.appendChild(tempCell);
